...
def test_GitHubRequest_fail():
    with pytest.raises(
        FileExists, match="CHANGELOG.md already exists at " + os.getcwd() + ". Use --force to override."
    ):
        GitHubRequest()
...
def test_GitHubRequest_fail_2():
    with pytest.raises(
        InvalidToken,
        match="Oops! GitHub requires you to generate a private token to get the details. See "
        "https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/ "
        "for more information.",
    ):
        GitHubRequest(force=True)
...
class TestGitHubRequestInit(unittest.TestCase):
    ...
    def test_init_condition(self):
        local_car = GitHubRequest(
            force=True, token="hello", repo_url="https://github.com/akshaybabloo/release-exporter"
        )
...
class TestGitHubRequestFail(unittest.TestCase):
    ...
    def test_total_number_releases(self):
        with pytest.raises(KeyError, match="Wrong credentials given. Please check if you have the correct token."):
            self.github_request_class._total_number_releases()

    def test_releases(self):
        with pytest.raises(KeyError, match="Wrong credentials given. Please check if you have the correct token."):
            self.github_request_class.releases()


class TestGitHubRequest(unittest.TestCase):
    def setUp(self):
        self.github_request_class = GitHubRequest(
            force=True, token=os.environ["GITHUB_TOKEN"], repo_url="https://github.com/akshaybabloo/release-exporter"
        )
...
def test_GitLabRequest_fail_2():
    with pytest.raises(
        InvalidToken,
        match="Oops! GitLab requires you to generate a private token to get the details. See "
        "https://docs.gitlab.com/ce/user/profile/personal_access_tokens.html "
        "for more information.",
    ):
        GitLabRequest(force=True)
...
class TestGitLabRequestInit(unittest.TestCase):
    ...
    def test_init_condition(self):
        local_car = GitHubRequest(force=True, token="hello", repo_url="https://gitlab.com/akshaybabloo/test-releases")
...
class TestGitLabRequestFail(unittest.TestCase):
    ...
    def test_repo_id(self):
        with pytest.raises(KeyError, match="Wrong credentials given. Please check if you have the correct token."):
            self.gitlab_request_class._repo_id()

    def test_releases(self):
        with pytest.raises(KeyError, match="Wrong credentials given. Please check if you have the correct token."):
            self.gitlab_request_class.releases()


class TestGitLabRequest(unittest.TestCase):
    def setUp(self):
        self.gitlab_request_class = GitLabRequest(
            force=True, token=os.environ["GITLAB_TOKEN"], repo_url="https://gitlab.com/akshaybabloo/test-releases"
        )
...
def test_GitLab_repo_id():
    gitlab_request_class = GitLabRequest(
        force=True, token=os.environ["GITLAB_TOKEN"], repo_url="https://gitlab.com/akshaybabloo/public"
    )
