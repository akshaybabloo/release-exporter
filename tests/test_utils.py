...
def test_get_repo_url_info_fail():
    with pytest.raises(
        ParserError, match="Git config file does not exist please provide the repository url by using --url."
    ):
        get_repo_url_info(location="yo")
...
def test_get_repo_url_info_fail_2():
    ...
    with pytest.raises(
        ...
        match="There seems to be a duplicate section in your config. Try giving the repository URL by using --url.",
    ):
        get_repo_url_info(location=t)
...
def test_date_convert_fail():
    with pytest.raises(ValueError, match="Unknown string format"):
        date_convert("10-10-2019T12:12:12z")
